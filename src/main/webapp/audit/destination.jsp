...
<body class="layui-view-body">
     <div class="layui-content" id="box" style="display:none">
      ...
      <div class="layui-content" id="detail" style="display:none">
        <form class="layui-form" action="" lay-filter="detail">
           <div class="layui-form-item">
             <label class="layui-form-label">攻略</label>
                 <div class="layui-input-block">
                   <textarea id="checkDetail" name="content" style="display: none;" placeholder="请输入攻略"></textarea>
                 </div>
           </div>
       </form>
     </div>
    ...
    layui.use('table', function(){
  	        ...
  	        ,content: $("#detail"),
  	         success:function(layero, index){
  	        	  layui.use('layedit', function(){
  	                 ...
  	                 layedit.build('checkDetail'); //建立编辑器
  	                 layedit.setContent(index,data.content)
  	               });
  	         }
    	   ,yes:function(index){
    		   toExamine(data.id,-1,index)
    	   }
    	   ,btn2:function(index){
    		   toExamine(data.id,1,index) 
    	   }
       ...
       //审核
       function toExamine(id,status,index){
    	   $.ajax({
               url:"<%=basePath%>strategy/addOrUpdate",
               type:'post',//method请求方式，get或者post
               dataType:'json',//预期服务器返回的数据类型
